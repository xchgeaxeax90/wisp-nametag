...
LIB_CSOURCES   := $(shell find $(LIB_TOP)/$(LIB_CDIR) -name '*.c')
LIB_RELS       = $(LIB_CSOURCES:$(LIB_TOP)/%.c=$(BDIR)/%.rel)
LIB_FWLIB      := fw_stc8.lib
LIB_INCFLAGS   := -I$(LIB_TOP)/$(LIB_INCLUDE)
...
$(BDIR)/$(LIB_CDIR)/%.rel: $(LIB_TOP)/$(LIB_CDIR)/%.c
	@printf "  CC $<\n"
	@mkdir -p $(dir $@)
